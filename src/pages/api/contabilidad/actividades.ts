...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  try {
    ...
    if (!nombre || !fecha_inicio) {
      return redirect('/contabilidad?error=' + encodeURIComponent('Nombre y fecha_inicio son requeridos'));
    }

    // 4. Inserción
    const { data, error: insertError } = await supabase
      .from('actividades')
      .insert([{ nombre, descripcion, fecha_inicio, fecha_fin, estado, user_id: user.id }])
      .select()
      .single();

    if (insertError) {
      console.error("[API] Error al insertar actividad:", insertError);
      return redirect('/contabilidad?error=' + encodeURIComponent('Error al guardar la actividad'));
    }

    console.log("[API] Actividad insertada exitosamente:", data.id);

    return redirect('/contabilidad?success=' + encodeURIComponent('Actividad registrada exitosamente ✅'));

  } catch (error) {
    console.error("[API] Error inesperado:", error);
    return redirect('/contabilidad?error=' + encodeURIComponent('Error al registrar la actividad'));
  }
};