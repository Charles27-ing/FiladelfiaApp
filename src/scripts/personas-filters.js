// src/scripts/personas-filters.js - VERSIÓN CON EXPOSICIÓN GLOBAL CORREGIDA
 ...
 */
function initializePersonasFilters(initialPersonas) {
  ...
  function populateSedeFilter(personas) {
    ...
    const sedes = [...new Set(personas.map(p => p.sedes).filter(Boolean))];
    sedes.forEach(sede => {
  ...
  function renderPersonasDesktop(personasToRender) {
    ...
    personasToRender.forEach(person => {
      ...
      let edadTexto = '';
      if (person.edad) {
        edadTexto = `${person.edad} años`;
      } else if (person.fecha_nacimiento) {
        const fechaNac = new Date(person.fecha_nacimiento);
        const hoy = new Date();
      ...
      const personRowHtml = `
        <tr class="hover:bg-gray-50">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
              <div class="flex-shrink-0 h-12 w-12">
                <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200" src="${photoSrc}" alt="Foto de ${person.nombres}">
              </div>
              <div class="ml-4">
                <div class="flex items-center">
                ...
                ${edadTexto ? `<div class="text-xs text-gray-400">${edadTexto}</div>` : ''}
              </div>
            </div>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="text-sm text-gray-500">${person.departamento || ''} ${person.municipio ? '- ' + person.municipio : ''}</div>
            <div class="text-xs text-gray-400">${person.direccion || ''}</div>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <button onclick="viewPerson('${person.id}')" class="text-green-600 hover:text-green-900 mr-2" title="Ver detalles">
            ...
            <button onclick="editPerson('${person.id}')" class="text-blue-600 hover:text-blue-900 mr-2" title="Editar">
              <svg class="w-4 h-4 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
              </svg>
            </button>
            <button onclick="deletePerson('${person.id}', '${person.nombres} ${person.primer_apellido}')" class="text-red-600 hover:text-red-900" title="Eliminar">
              <svg class="w-4 h-4 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden m-4">
          <div class="p-4">
            <div class="flex items-center space-x-4">
              <img class="h-16 w-16 rounded-full object-cover border-2 border-gray-200" src="${photoSrc}" alt="Foto de ${person.nombres}">
              <div class="flex-1">
                <div class="flex items-center">
                ...
                ${edadTexto ? `<p class="text-xs text-gray-400">${edadTexto}</p>` : ''}
              </div>
            </div>
            <div class="mt-4 space-y-2">
              <p class="text-sm text-gray-600"><span class="font-medium">Sede:</span> ${sedeName}</p>
              <p class="text-sm text-gray-600"><span class="font-medium">Ubicación:</span> ${person.departamento || ''} ${person.municipio ? '- ' + person.municipio : ''}</p>
              <div>
...
window.initializePersonasFilters = initializePersonasFilters;
